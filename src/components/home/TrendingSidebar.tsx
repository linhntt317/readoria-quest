...
export function TrendingSidebar({ mangaList }: TrendingSidebarProps) {
  const { t } = useTranslation();

  const trending = mangaList
    .slice()
      ...
      <Card className="p-6 bg-gradient-to-br from-card to-secondary/20 border-border/50">
        ...
        <div className="space-y-3">
          {trending.map((manga, index) => {
            const rank = index + 1;
            return (
              <Link key={manga.id} href={`/truyen/${manga.id}`}>
                <div className="flex items-center gap-3 p-3 rounded-lg hover:bg-secondary/50 transition-colors cursor-pointer group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {manga.views.toLocaleString()} {t.common.views}
                    </p>
                  </div>
      ...
      <Card className="p-6 bg-gradient-to-br from-primary/10 to-accent/5 border-primary/20">
        ...
        <div className="space-y-2 text-sm text-muted-foreground">
          <p className="flex items-start gap-2">
            ...
            <span>{mangaList.length} truyện đang có trên hệ thống</span>
          </p>
          <p className="flex items-start gap-2">
            <span className="inline-block w-1.5 h-1.5 rounded-full bg-primary mt-1.5" />
